...
public class UsuarioServiceTest {
    ...
    @Test
    public void actualizarPerfilActualizaNombreYEmail() {
        // GIVEN
        // Un usuario registrado en la BD
        Long idUsuario = addUsuarioBD();

        UsuarioData datosActualizados = new UsuarioData();
        datosActualizados.setNombre("Nombre Actualizado");
        datosActualizados.setEmail("nuevo@ua");

        // WHEN
        UsuarioData resultado = usuarioService.actualizarPerfil(idUsuario, datosActualizados);

        // THEN
        // El DTO devuelto tiene los nuevos datos
        assertThat(resultado.getNombre()).isEqualTo("Nombre Actualizado");
        assertThat(resultado.getEmail()).isEqualTo("nuevo@ua");

        // Y si volvemos a consultar a la BD, los cambios se han guardado
        UsuarioData usuarioBD = usuarioService.findById(idUsuario);
        assertThat(usuarioBD.getNombre()).isEqualTo("Nombre Actualizado");
        assertThat(usuarioBD.getEmail()).isEqualTo("nuevo@ua");
    }

    @Test
    public void cambiarPasswordConPasswordActualCorrecta() {
        // GIVEN
        Long idUsuario = addUsuarioBD();

        // Comprobamos que inicialmente el login con "123" es correcto
        UsuarioService.LoginStatus loginAntes =
                usuarioService.login("user@ua", "123");
        assertThat(loginAntes).isEqualTo(UsuarioService.LoginStatus.LOGIN_OK);

        // WHEN
        usuarioService.cambiarPassword(idUsuario, "123", "nuevaClave");

        // THEN
        // La antigua contraseña ya no sirve
        UsuarioService.LoginStatus loginAntigua =
                usuarioService.login("user@ua", "123");
        assertThat(loginAntigua).isEqualTo(UsuarioService.LoginStatus.ERROR_PASSWORD);

        // La nueva contraseña sí permite el login
        UsuarioService.LoginStatus loginNueva =
                usuarioService.login("user@ua", "nuevaClave");
        assertThat(loginNueva).isEqualTo(UsuarioService.LoginStatus.LOGIN_OK);
    }

    @Test
    public void cambiarPasswordConPasswordActualIncorrectaLanzaExcepcion() {
        // GIVEN
        Long idUsuario = addUsuarioBD();

        // WHEN, THEN
        // La contraseña actual no coincide -> se lanza UsuarioServiceException
        Assertions.assertThrows(UsuarioServiceException.class, () -> {
            usuarioService.cambiarPassword(idUsuario, "passwordIncorrecta", "nuevaClave");
        });
    }
}